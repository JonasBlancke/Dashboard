...
`cities.forecast.yaml` **is** the city list. Both the daily GitHub Action and
the local `scripts/refresh_forecasts.sh` iterate it — no code changes to add a
city.

1. **Add a block to `cities.forecast.yaml`**: `ml_urbanheat_city_name`,
   `display_name`, `timezone`, `center`, `merit_height_m` (one cached MERIT-DEM
   lookup at the centre — skips Earth Engine), `has_local_observations`,
   `enabled: true`. Optional per-city overrides: `spatial_sim` (where its V29
   resampled TIFs live if not `V29_forecast`), `simulation_name` + `config_path`
   (a city with big elevation relief inside its AOI uses
   `V29_forecast_lrc_pixel` — a per-pixel lapse-rate model — instead of the
   city-centre-only `V29_forecast`), `value_domain_c` (pin the colour scale).
2. **In `ML-UrbanHeat`**, for that city:
   - `cities/<Name>/processed_data/<sim>/spatial/` — the resampled predictor
     TIFs + `default_grid.tif` (from `run_prepare_spatial.py`)
   - `cities/<Name>/raw_data/AOI.geojson` — **required**, clips the prediction
     grid to the modelled area
   - `cities/<Name>/raw_data/ZOI.geojson` — optional, drawn as a red outline
3. **Run it**:
   ```
   bash scripts/refresh_forecasts.sh              # every enabled city, then commit + push
   bash scripts/refresh_forecasts.sh <id>         # just the new one
   SKIP_MODEL=1 bash scripts/refresh_forecasts.sh <id>   # reuse the newest NetCDF
   NO_PUSH=1    bash scripts/refresh_forecasts.sh <id>   # build only; review with `python backend/app.py`
   ```
   Needs the ML conda env active. The city picker in the frontend appears
   automatically — it reads `manifest.json`'s `cities[]`.
4. **For the daily GitHub Action** (optional — only if you want CI to keep this
   city fresh too): `bash scripts/stage_v29_assets.sh <id>` uploads its spatial
   TIFs + AOI/ZOI + (per distinct sim) model/config to the `v29-assets` Release.
